...
describe("findAllwithFilter", function () {
  ...
  test("works: by min employees", async function () {
    let companies = await Company.findAllWithFilter({ minEmployees: 2 });
    expect(companies).toEqual([
      {
        handle: "c2",
        name: "C2",
  ...
  test("works: by max employees", async function () {
    let companies = await Company.findAllWithFilter({ maxEmployees: 2 });
    expect(companies).toEqual([
      {
        handle: "c1",
        name: "C1",
  ...
  test("works: by name", async function () {
    let companies = await Company.findAllWithFilter({ name: "1" });
    expect(companies).toEqual([
      {
        handle: "c1",
        name: "C1",
  ...
  test("works: empty list on nothing found", async function () {
    let companies = await Company.findAllWithFilter({ name: "nope" });
    expect(companies).toEqual([]);
  });

  test("bad request if invalid min > max", async function () {
    try {
      await Company.findAllWithFilter({ minEmployees: 10, maxEmployees: 1 });
      fail();
    } catch (err) {
      expect(err instanceof BadRequestError).toBeTruthy();
    }
